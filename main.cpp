...
int main(int argc, char* argv[])
{
    ...
    do
    {
        try
        {
            ...
            if(thread_count < 2)   //至少两个线程，一个业务，一个日志输出
            {
                thread_count = 2;
            }

            boost::asio::io_service ioc_log;
            boost::asio::strand<boost::asio::io_service::executor_type> strand_log = boost::asio::make_strand(ioc_log);//用来同步日志输出
            ...
            tv[thread_count - 1] = std::make_shared<std::thread>(
                        [&ioc_log](void){
                for(;;)
                {
                    try
